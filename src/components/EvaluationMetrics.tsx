...
export function EvaluationMetrics({ project }: EvaluationMetricsProps) {
  ...
  const quantCards = [
    { label: 'TASKS COMPLETED', val: `${metrics.completedTasks}/${metrics.totalTasks}`, status: metrics.completedTasks === metrics.totalTasks ? 'PASS' : 'IN_PROGRESS' },
    { label: 'REQUIREMENTS SATISFIED', val: `${metrics.satisfiedRequirements}/${metrics.totalRequirements}`, status: 'PASS' },
    { label: 'TEST PASS RATE', val: `${metrics.passingTests}/${metrics.totalTests}`, status: metrics.failingTests === 0 ? 'PASS' : 'FAIL' },
    { label: 'TEST COVERAGE', val: `${metrics.coveragePct}%`, status: metrics.coveragePct >= 80 ? 'PASS' : 'IN_PROGRESS' },
    { label: 'BUILD STATUS', val: metrics.buildStatus, status: metrics.buildStatus === 'PASSED' ? 'PASS' : 'IN_PROGRESS' },
    { label: 'TYPE ERRORS', val: `${metrics.typeErrors}`, status: metrics.typeErrors === 0 ? 'PASS' : 'FAIL' },
    { label: 'LINT WARNINGS', val: `${metrics.lintErrors}`, status: metrics.lintErrors === 0 ? 'PASS' : 'FAIL' },
    { label: 'SECURITY VULNS', val: `${metrics.securityIssues}`, status: metrics.securityIssues === 0 ? 'PASS' : 'FAIL' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 sm:p-5 rounded-lg bg-[#0a0a0a]/50 border border-white/10 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-1.5">
          <div className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-[10px] font-mono font-bold tracking-wider">
            ...
            <span>AI PRINCIPAL SOFTWARE ARCHITECT AUDIT</span>
          </div>
          <h2 className="text-xl font-bold font-mono text-white tracking-tight">
            SYSTEM QUALITY & VERIFICATION REPORT
          </h2>
          <p className="text-[11px] text-white/50 max-w-xl leading-relaxed font-sans">
            Multi-dimensional evaluation covering architectural soundness, Zero-Trust security invariants, regression test coverage, and strict requirement compliance.
          </p>
        </div>
      ...
      <div className="space-y-2.5">
        <h3 className="text-[10px] font-mono uppercase tracking-widest text-white/70 font-bold flex items-center space-x-2">
          ...
          <span>QUANTITATIVE INVARIANT CHECKS</span>
        </h3>
      ...
      <div className="space-y-2.5">
        <h3 className="text-[10px] font-mono uppercase tracking-widest text-white/70 font-bold flex items-center space-x-2">
          ...
          <span>QUALITATIVE AI ASSESSMENT PILLARS</span>
        </h3>
