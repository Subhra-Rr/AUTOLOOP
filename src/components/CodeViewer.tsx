  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 h-full">
      ...
      <div className="lg:col-span-1 rounded-lg bg-[#0a0a0a]/50 border border-white/10 p-3 space-y-3 flex flex-col">
        ...
        <div className="p-2 rounded bg-black/40 border border-white/5 text-[10px] font-mono text-white/40 space-y-1">
      ...
      <div className="lg:col-span-3 rounded-lg bg-black/40 border border-white/5 flex flex-col overflow-hidden shadow-2xl font-mono">
        {/* Editor Title Bar */}
        <div className="h-8 flex items-center justify-between px-3 bg-white/5 border-b border-white/5 select-none">
          <div className="flex items-center space-x-2 text-xs">
            <FileCode2 className="w-3.5 h-3.5 text-cyan-400" />
            <span className="font-bold text-white text-[11px]">{currentFile?.path}</span>
        ...
        {/* Code / Diff Body */}
        <div className="flex-1 overflow-y-auto p-3 text-[11px] leading-relaxed select-text bg-[#050505]/95">
          {!currentFile ? (
            ...
            <div className="space-y-3">
              <div className="p-2.5 rounded bg-red-500/10 border border-red-500/30 text-red-300 text-xs flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Wrench className="w-3.5 h-3.5 text-red-400" />
                  <span>
                    Automated Patch applied during <strong>Failure-Repair Cycle</strong>
                  </span>
                </div>
              ...
              <div className="space-y-1">
                ...
                <pre className="p-2.5 rounded bg-red-500/10 border border-red-500/20 text-red-300 overflow-x-auto whitespace-pre-wrap font-mono text-[11px]">
                  {repairDiff.before}
                </pre>
              </div>
              ...
              <div className="space-y-1">
                ...
                <pre className="p-2.5 rounded bg-green-500/10 border border-green-500/20 text-green-300 overflow-x-auto whitespace-pre-wrap font-mono text-[11px]">
                  {repairDiff.after}
                </pre>
              </div>
            </div>
            ...
            <div className="relative">
              {currentFile.content.split('\n').map((line, idx) => (
                <div key={idx} className="flex hover:bg-white/5 px-1 py-0.5 rounded">
                  <span className="text-white/30 select-none w-10 text-right pr-4 text-[10px] font-mono">
                    {idx + 1}
                  </span>
                  <span className="text-white/80 flex-1 whitespace-pre-wrap font-mono">
                    {line || ' '}
                  </span>
                </div>
              ))}
        ...
        <div className="h-7 px-3 bg-white/5 border-t border-white/5 text-[9px] text-white/40 flex items-center justify-between select-none">
          <span>{currentFile ? `UTF-8 • ${currentFile.language.toUpperCase()} • ${currentFile.content.split('\n').length} LINES` : 'WORKSPACE EMPTY'}</span>
          <span className="text-cyan-400">ZERO_TRUST_SANDBOX_PROTECTED</span>
        </div>
      </div>
    </div>
  );
